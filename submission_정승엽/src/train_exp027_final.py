"""exp_027 final -- retrain the seed{42,1} combo on the FULL 2019-2024
train.csv with the champion feature set PLUS the ported current-season
state features (src/season_state_features.py, from a teammate's
EXP_056 -- see experiments/exp_027_season_state_features.md). Single
seed=42 alone (818.79) slightly beat the 2-seed average (818.30,
experiments/exp027_seedbag_run_log.txt) but the gap is within this
project's established noise floor (<2 points) -- kept the standard
2-seed{42,1} bagging for consistency/variance-reduction with every
other champion generation in this project.

Saves:
  model/catboost_model_seed42.cbm
  model/catboost_model_seed1.cbm
  model/model_meta.pkl              (model_type="catboost_seedbag")
  model/trackman_pitcher_lookup.csv (unchanged from exp_010, shrink_k=50)
  model/season_state_pitcher_lookup.csv  (NEW, latest_season_baselines)
  model/season_state_batter_lookup.csv   (NEW, latest_season_baselines)
"""
...
def build_season_state(df, train_only, entity):
    baselines = build_season_baselines(train_only, entity)
    priors = fit_season_state_priors(train_only, entity)
    return attach_season_state_features(df, baselines, priors, entity)
...
def main():
    ...
    df_val = build_season_state(df_val, train_only, "pitcher")
    df_val = build_season_state(df_val, train_only, "batter")
    ...
    train_final = build_season_state(train_final, train, "pitcher")
    train_final = build_season_state(train_final, train, "batter")
    ...
    pitcher_season_lookup = latest_season_baselines(train, "pitcher")
    batter_season_lookup = latest_season_baselines(train, "batter")
    pitcher_season_priors = fit_season_state_priors(train, "pitcher")
    batter_season_priors = fit_season_state_priors(train, "batter")
    pitcher_season_lookup.to_csv(os.path.join(MODEL_DIR, "season_state_pitcher_lookup.csv"), index=False, encoding="utf-8-sig")
    batter_season_lookup.to_csv(os.path.join(MODEL_DIR, "season_state_batter_lookup.csv"), index=False, encoding="utf-8-sig")
    print(f"저장: season_state_{{pitcher,batter}}_lookup.csv ({pitcher_season_lookup.shape}, {batter_season_lookup.shape})")
    ...
    joblib.dump(
        {
            ...
            "season_state_cols": SEASON_STATE_COLS,
            "season_state_pitcher_priors": pitcher_season_priors,
            "season_state_batter_priors": batter_season_priors,
            "season_state_shrink_k": 20,
            "seed_model_files": seed_model_files,
            "exp_id": "exp_027_season_state_seedbag42_1",
        },
