"""exp_017 final -- retrain the CatBoost + tuned-LightGBM(slow_lr) blend
(cb=0.76/lgb=0.24, from exp017_run_log.txt's coarse_fine blend search)
on the FULL 2019-2024 train.csv. Iteration counts reused directly:
CatBoost->696 (same seed42/params as exp_007/010/013, deterministic),
LightGBM(slow_lr)->442 (exp017_run_log.txt val best_iter(441)+1).
"""
...
SEED = 42
CB_ITERATIONS = 696
LGB_ROUNDS = 442  # exp017_run_log.txt slow_lr val best_iter(441)+1
LGB_OVERRIDES = dict(num_leaves=63, learning_rate=0.01, min_child_samples=200, subsample=0.8, colsample_bytree=0.8, reg_lambda=0.0)
WEIGHTS = {"catboost": 0.76, "lightgbm": 0.24}
...
def main():
    ...
    print("=" * 80)
    cb_params = dict(
        iterations=CB_ITERATIONS, learning_rate=0.03, depth=6, l2_leaf_reg=3.0,
    ...
    t = time.time()
    clf_cb.fit(train_pool_cb)
    print(f"CatBoost 완료 :: {time.time() - t:.1f}s")

    print()
    print("=" * 80)
    print("3. LightGBM(slow_lr) 전체 재학습")
    print("=" * 80)
    cat_encoder = OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=-1)
    X_enc = X.copy()
    X_enc[CAT_COLS] = cat_encoder.fit_transform(X_enc[CAT_COLS]).astype(int)
    lgb_params = dict(objective="binary", metric="binary_logloss", seed=SEED, num_threads=-1, verbosity=-1)
    ...
    for stale in ["catboost_model_seed42.cbm", "catboost_model_seed1.cbm", "catboost_model_seed7.cbm"]:
        p = os.path.join(MODEL_DIR, stale)
        if os.path.exists(p):
            os.remove(p)
            print(f"제거: {p} (이전 실험 아티팩트 정리)")

    cb_file = "catboost_model.cbm"
    lgb_file = "lgbm_booster.txt"
    clf_cb.save_model(os.path.join(MODEL_DIR, cb_file))
    ...
    joblib.dump(
        {
            ...
            "members": [
                {"type": "catboost", "file": cb_file, "weight": WEIGHTS["catboost"]},
                {"type": "lightgbm", "file": lgb_file, "weight": WEIGHTS["lightgbm"]},
            ],
            "lgb_cat_encoder": cat_encoder,
            "exp_id": "exp_017_lgbm_tuned_blend",
        },
        os.path.join(MODEL_DIR, "model_meta.pkl"),
        compress=3,
    )
    print(f"저장: {MODEL_DIR}/{cb_file}, {MODEL_DIR}/{lgb_file}, {MODEL_DIR}/model_meta.pkl")
    print("\n완료.")
