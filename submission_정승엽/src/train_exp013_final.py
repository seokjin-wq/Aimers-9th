"""exp_013 final -- retrain the CatBoost+LightGBM blend
(cb=0.83/lgb=0.17, from experiments/exp013_run_log.txt's coarse_fine
blend search) on the FULL 2019-2024 train.csv. Iteration counts are
reused directly from the validation run (695+1 for CatBoost, 154+1 for
LightGBM -- same seed/data/params, deterministic).

Saves model_type="ensemble" (import lightgbm before catboost -- see
CLAUDE.md's documented DLL-load-order fix).
"""
...
SEED = 42
CB_ITERATIONS = 696   # exp013_run_log.txt val best_iter(695)+1
LGB_ROUNDS = 155      # exp013_run_log.txt val best_iter(154)+1
WEIGHTS = {"catboost": 0.83, "lightgbm": 0.17}
    ...
    joblib.dump(
        {
            ...
            "exp_id": "exp_013_catboost_lightgbm_blend",
        },
