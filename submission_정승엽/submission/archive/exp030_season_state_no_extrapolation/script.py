...
from season_state_features import attach_season_state_features_flat
...
def build_features_for_inference(df, shrinkage_priors, trackman_lookup, trackman_cols,
                                  ...
                                  season_state_lookups=None):
    """row_id 제거 + src/features.py의 build_features/apply_shrinkage +
    exp_007 trackman 투수 단위 피처 + (exp_027부터) 이번시즌 상태 피처.
    다른 행 정보는 쓰지 않음.

    cat_encoder가 주어지면(LightGBM) 학습 때와 동일한 인코더로 범주형을
    정수 인코딩한다 (category dtype은 소량 배치 추론에서 LightGBM 네이티브
    크래시를 일으키므로 사용하지 않음). CatBoost는 원본 문자열 범주형을
    그대로 받으므로 인코딩하지 않는다.

    season_state_lookups가 주어지면(exp_027 이후 아카이브만) 투수/타자
    각각의 `season_state_features.attach_season_state_features_flat`을
    적용한다 -- test 행 자신의 asof_{entity}_n/rate(진짜로 2025 시즌
    내내 갱신되는 값, `data/test.csv`로 직접 확인함)에서 2024시즌 말
    누적값(고정 lookup)을 빼서 "이번 시즌만의" 통계를 복원한다."""
    ...
    if season_state_lookups is not None:
        for entity, (lookup, priors, shrink_k) in season_state_lookups.items():
            df = attach_season_state_features_flat(df, lookup, priors, entity, shrink_k=shrink_k)
...
def main():
    ...
    season_state_cols = meta.get("season_state_cols", [])
    season_state_lookups = None
    if season_state_cols:
        season_state_shrink_k = meta.get("season_state_shrink_k", 20)
        season_state_lookups = {}
        for entity in ("pitcher", "batter"):
            lookup = pd.read_csv(os.path.join(MODEL_DIR, f"season_state_{entity}_lookup.csv"), encoding="utf-8-sig")
            priors = meta[f"season_state_{entity}_priors"]
            season_state_lookups[entity] = (lookup, priors, season_state_shrink_k)
            print(f" season_state {entity} lookup: {lookup.shape} (shrink_k={season_state_shrink_k})")
    ...
    X = build_features_for_inference(
        ...
        cat_encoder=cat_encoder, season_state_lookups=season_state_lookups,
    )[all_features]
    ...
    # exp_029: count-state(balls_before, strikes_before)별 시즌 추세
    # 보정 -- exp_022/023의 전역 보정과 다른 축(count-state별 잔차
    # drift)을 잡는 부가 보정. 순수 행-로컬 dict lookup(test 행 자신의
    # balls_before/strikes_before만 사용).
    count_trend = meta.get("count_trend")
    if count_trend is not None:
        group_cols = count_trend["group_cols"]
        trend = count_trend["trend"]
        strength = count_trend["strength"]
        if len(group_cols) == 1:
            keys = test[group_cols[0]].tolist()
        else:
            keys = list(zip(*[test[c] for c in group_cols]))
        correction = np.array([trend.get(k, 0.0) for k in keys])
        preds = preds + strength * correction
