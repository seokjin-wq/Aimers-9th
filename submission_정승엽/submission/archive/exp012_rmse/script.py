...
def main():
    ...
    if model_type == "lightgbm":
        ...
        # [{"type":..., "file":..., "weight":...}, ...]. lightgbm 멤버가
        # 있으면 그 모델이 학습 때 쓴 OrdinalEncoder(meta["lgb_cat_encoder"])로
        # 범주형을 정수 인코딩한 별도 피처 행렬을 그 멤버에만 사용한다
        # (catboost 계열은 원본 문자열 범주형을 그대로 씀).
        import catboost as cb
        members = []
        need_lgb = any(m["type"] == "lightgbm" for m in meta["members"])
        if need_lgb:
            import lightgbm as lgb
            lgb_cat_encoder = meta["lgb_cat_encoder"]
        for m in meta["members"]:
            if m["type"] == "catboost":
                obj = cb.CatBoostClassifier()
                obj.load_model(os.path.join(MODEL_DIR, m["file"]))
            elif m["type"] == "catboost_regressor":
                obj = cb.CatBoostRegressor()
                obj.load_model(os.path.join(MODEL_DIR, m["file"]))
            elif m["type"] == "lightgbm":
                obj = lgb.Booster(model_file=os.path.join(MODEL_DIR, m["file"]))
            else:
                raise ValueError(f"알 수 없는 ensemble member type: {m['type']}")
            members.append((m["type"], obj, m["weight"]))
        cat_encoder = None
    else:
    ...
    if len(X) == 0:
        ...
        for kind, obj, w in members:
            if kind == "lightgbm":
                if X_enc is None:
                    X_enc = X.copy()
                    X_enc[CAT_COLS] = lgb_cat_encoder.transform(X_enc[CAT_COLS]).astype(int)
                p = obj.predict(X_enc)
            elif kind == "catboost_regressor":
                p = np.clip(obj.predict(X), 0.0, 1.0)
            else:
                p = obj.predict_proba(X)[:, 1]
            preds = preds + w * p
    else:
